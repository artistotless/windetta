...
public abstract class ConfigurableLobbyPlugin : ILobbyPlugin
{
    ...
    protected ConfigurableLobbyPlugin()
    {
        RequirementsDefinition = SetupRequirements(
            new RequirementsDefinitionBuilder());
    }

    internal ConfigurableLobbyPlugin WithValues(IDictionary<string, string> values)
    {
    ...
    protected T GetRequirementValue<T>(string nameOfRequirement)
    {
        ...
        if (result is null)
            throw LobbyPluginException.RequirementValueInvalid;
