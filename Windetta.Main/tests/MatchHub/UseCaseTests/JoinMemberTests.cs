...
public class JoinMemberTests
{
    ...
    {
        // arrange
        IMatchHub hub = new Fixture().Create<FakeMatchHub>();
    ...
    public async Task ShouldExecuteFiltersIfDefined()
    {
        ...
        var hubMock = new Mock<FakeMatchHub>(options, null);
        hubMock.Setup(x => x.GetJoinFilters()).Returns(new List<IJoinFilter>() { new AlwaysFalseJoinFilter() });
        ...
        //assert
        exception.ErrorCode.ShouldNotBeNull(Errors.Main.JoinFilterValidationFail);
